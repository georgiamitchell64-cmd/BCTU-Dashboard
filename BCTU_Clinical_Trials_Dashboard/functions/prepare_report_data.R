...
# Site helpers used by the site table below. They live here, not in
# projection_math.R, so report building works when only this file is loaded
# (tests/ and scripts/render_report.R source a subset of the app).
...
#
# Per-site recruitment history, the input to the Sites tab tile and the
# report's "Actual / mo" column:
#
...
prepare_report_data <- function(df,
  ...
  if (!is.null(reg)) {
    ...
    # Sized to the sites found: with nobody recruited yet there are none, and a
    # length-1 default beside a zero-length site_name is an error, not a 0.
    n0 <- nrow(all_time_counts)
    site_reg <- data.frame(
      site_name      = all_time_counts$site_name,
      stage          = rep(NA_character_, n0),
      target         = rep(NA_integer_, n0),
      monthly_target = rep(NA_integer_, n0),
