...
<div class="pf-donut-ctr"><b>%d</b><small>TRIALS</small></div>',
  ...
  output$wiz_tp_fields_ui <- renderUI({
    ...
    tagList(
      div(style = "display:grid;grid-template-columns:150px 1fr 110px 110px;gap:10px;
                   margin-top:10px;font-size:11px;font-weight:600;color:#58595B;
  ...
  output$wiz_cb_status <- renderUI({
    ...
    if (!isTRUE(st$ok))
      return(div(class = "nt-hint", style = "color:#C20019;", st$msg))
    ...
    if (!length(notes)) {
      return(div(style = "padding:40px 20px;text-align:center;color:#8A8A8C;",
                 ...
                 div(style = "font-size:12px;",
                     "No active notifications. Warnings about recruitment
                      pace, stalled recruitment, sites not recruiting or data
                      completeness will show up here.")))
    }
